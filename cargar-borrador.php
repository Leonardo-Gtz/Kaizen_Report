<?php
// ===================================
// ARCHIVO: guardar-borrador.php (CORREGIDO BASADO EN TU VERSIÓN)
// ===================================
...
try {
    ...
    if (empty($tema)) {
        throw new Exception('El tema es obligatorio');
    }
    ...
    // Preparar statement para insertar borrador (con estado = 'borrador' y estados adicionales)
    $sqlRpt = "INSERT INTO reportes 
        (tema, fecha, imagen_anterior, descripcion_anterior, imagen_mejora, descripcion_mejora, analisis_riesgo, archivo_riesgo, estado, estado_jefe, estado_gerente, estado_director, fecha_creacion) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'borrador', 'pendiente', 'pendiente', 'pendiente', NOW())";
    ...
    if (!$stmt->execute()) {
        throw new Exception('Error al guardar el borrador: ' . $stmt->error);
    }

    $idRpt = $conexion->insert_id;
    $stmt->close();

    // Insertar participantes (CORREGIDO: agregando EmpId)
    if (!empty($part)) {
        $sqlPart = "INSERT INTO reporte_participantes (id_reporte, id_participante, nombre, departamento, EmpId) VALUES (?, ?, ?, ?, ?)";
        ...
        foreach ($part as $p) {
            ...
            // CORREGIDO: Usar EmpId o id como fallback
            ...
            $stmtPart->bind_param('issss', $idRpt, $idP, $nom, $dept, $empId);
    ...
    echo json_encode(array(
        ...
        'id_reporte' => $idRpt,
        'participantes_guardados' => count($part),
